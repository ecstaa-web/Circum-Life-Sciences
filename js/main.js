...
(function() {
  ...
  // ===== WordPress forms & REST API =====
  function getCircumWp() {
    return window.circumWp || null;
  }

  function isWordPressSite() {
    var wp = getCircumWp();
    return !!(wp && wp.ajaxUrl && wp.isWp);
  }

  function getRestUrl(path) {
    var wp = getCircumWp();
    if (!wp || !wp.restUrl) return null;
    var base = wp.restUrl.replace(/\/$/, '');
    return base + (path.charAt(0) === '/' ? path : '/' + path);
  }

  function fetchCircumRest(path) {
    var url = getRestUrl(path);
    if (!url) return Promise.reject(new Error('REST unavailable'));
  ...
  function parseWpAjaxResponse(res) {
    return res.json().then(function (json) {
      if (!json || typeof json.success === 'undefined') {
        throw new Error('Invalid response');
      }
      if (!json.success) {
        var msg = (json.data && (json.data.message || json.data)) || 'error';
  ...
  function appendNewsletterFields(fd, form) {
    ...
    fd.append('company', (form.querySelector('[name="company"]') || {}).value || '');
    fd.append('role', (form.querySelector('[name="role"]') || {}).value || '');
    fd.append('lang', (form.querySelector('[name="lang"]') || {}).value || getCurrentLang());
    if (consentCb && consentCb.checked) fd.append('consent', '1');
    if (implicitConsent) fd.append('implicit_consent', '1');
    if (websiteEl) fd.append('website', websiteEl.value);

    form.querySelectorAll('input[name="interests[]"]:checked').forEach(function (cb) {
      fd.append('interests[]', cb.value);
    });
  }

  function submitViaWordPress(type, form) {
    var wp = getCircumWp();
    if (!wp || !wp.ajaxUrl) return Promise.reject(new Error('wordpress_required'));

    var fd = new FormData();
    fd.append('action', 'circum_' + type);
    fd.append('nonce', (wp.nonces && wp.nonces[type]) || '');

    if (type === 'newsletter') {
      appendNewsletterFields(fd, form);
    } else {
      new FormData(form).forEach(function (value, key) {
        if (key !== 'action' && key !== 'nonce') fd.append(key, value);
      });
      fd.set('lang', getCurrentLang());
    }

    return fetch(wp.ajaxUrl, {
      method: 'POST',
      body: fd,
      credentials: 'same-origin'
    }).then(parseWpAjaxResponse);
  }
  ...
  function initForms() {
    document.querySelectorAll('form[data-form]').forEach(function(form) {
      ...
      form.addEventListener('submit', function(e) {
        ...
        if (!isWordPressSite()) {
          showMessage(messageEl, L['form.error.wp'] || 'Les formulaires seront actifs une fois le site publié sur WordPress.', true);
          return;
        }
        ...
        submitViaWordPress(type, form).then(function (data) {
          if (submitBtn) { submitBtn.disabled = false; submitBtn.innerHTML = orig; }
          form.reset();
          var successKey = 'form.success.' + (type || 'default');
          var serverMsg = data && data.message;
          showMessage(messageEl, serverMsg || L[successKey] || L['form.success.default'] || 'Merci, votre demande a bien été envoyée.', false);
        }).catch(function (err) {
          if (submitBtn) { submitBtn.disabled = false; submitBtn.innerHTML = orig; }
          var errMsg = err && err.message;
          if (errMsg === 'wordpress_required') {
            showMessage(messageEl, L['form.error.wp'] || 'Les formulaires seront actifs une fois le site publié sur WordPress.', true);
            return;
          }
          showMessage(messageEl, errMsg || L['form.error.server'] || 'Erreur lors de l\'envoi. Veuillez réessayer.', true);
        });
      });
    });
  }
  ...
  function newsMediaUrl(filename) {
    ...
    var wp = getCircumWp();
    if (wp && wp.uploadsUrl) {
      return wp.uploadsUrl.replace(/\/$/, '') + '/' + filename;
    }
    return filename;
  }
  ...
  function initDynamicNews() {
    ...
    fetchCircumRest('circum/v1/news').then(function (items) {
  ...
  function initNewsArticle() {
    ...
    fetchCircumRest('circum/v1/news/' + encodeURIComponent(id)).then(function (article) {
  ...
  function initDynamicIssues() {
    ...
    fetchCircumRest('circum/v1/newsletter/issues').then(function (data) {
